...
export default function ReceitasVariaveisPage() {
  ...
  const getParcelaAtual = (item: ReceitaVariavel) => {
    const inicio = new Date(item.dataInicio);
    const diffMeses =
      (now.getFullYear() - inicio.getUTCFullYear()) * 12 + now.getMonth() - inicio.getUTCMonth();
    return item.parcelaAtual + diffMeses;
  };

  const isConfirmada = (item: ReceitaVariavel) => {
  ...
  const totalMensal = ativas.filter((i) => !isConfirmada(i)).reduce((sum, i) => sum + i.valorParcela, 0);

  const pendentes = ativas.filter((i) => !isConfirmada(i));
  const recebidas = ativas.filter((i) => isConfirmada(i));
        ...
        <div>
          ...
          {!loading && (
            <p className="text-sm text-gray-500 mt-1">
              A receber este mês: <span className="font-medium text-emerald-600">{formatCurrency(totalMensal)}</span>
            </p>
          )}
        </div>
        <button
