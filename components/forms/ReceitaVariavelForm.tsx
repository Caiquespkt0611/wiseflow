...
const schema = z.object({
  ...
  fonte: z.string().min(1, "Obrigatório"),
...
export type ReceitaVariavelFormData = z.infer<typeof schema>;

export type ReceitaVariavelPayload = ReceitaVariavelFormData & {
  parcelaAtual: number;
  valorTotal: number;
};

interface Props {
  defaultValues?: Partial<ReceitaVariavelFormData>;
  onSubmit: (data: ReceitaVariavelPayload) => Promise<void>;
  loading?: boolean;
}

export function ReceitaVariavelForm({ defaultValues, onSubmit, loading }: Props) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      <Field label="Descrição" error={errors.descricao?.message}>
        <input {...register("descricao")} className={inputClass} placeholder="Ex: Freela design" />
      </Field>

      <div className="grid grid-cols-2 gap-4">
        <Field label="Fonte" error={errors.fonte?.message}>
          <input {...register("fonte")} list="fontes-sugestoes" className={inputClass} placeholder="Ex: PIX, Boleto..." />
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field label="Parcelas restantes" error={errors.parcelasTotal?.message}>
          <input {...register("parcelasTotal")} type="number" min="1" className={inputClass} placeholder="Ex: 3" />
        </Field>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Field label="Próximo recebimento" error={errors.dataInicio?.message}>
